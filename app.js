 ...
 */
/** カード/チェックボックス/プルダウンの選択値を、記録用に日本語の表示名へ変換する */
function displayValueForRecord(question, rawValue) {
  ...
  if (question.type === "cards" || question.type === "select") {
    ...
    return option ? option.label.ja : rawValue;
  }
  if (question.type === "checkbox") {
    ...
    return values
      .map((v) => {
        ...
        return option ? option.label.ja : v;
      })
      .join(" / ");
  }
  if (question.type === "consent") {
    return rawValue ? "同意する" : "";
  }
  return rawValue;
}
...
function buildAnswerSummary() {
  ...
  flow.forEach((q) => {
    ...
    const value =
      q.type === "file" && rawValue
        ? { fileName: rawValue.name }
        : displayValueForRecord(q, rawValue);
...
function render() {
  ...
  // 質問や画面が切り替わったときだけ、ページの一番上にスクロールする
  // (カードやチェックボックスをタップしただけの再描画ではスクロールしない)
  const scrollKey = state.screen + ":" + state.stepIndex + ":" + state.editMode;
